	...
	return (
		<main className="tm-col-right fade-in-from-bottom">
			<div style={{display: 'flex', flexDirection: 'column'}}>
				...
				<h2 className="tm-content-title"><strong>Professional Summary</strong></h2>
					{/* Summary */}
					{professionalSummary.map((item, index) => {
						const last = professionalSummary.length - 1 === index;
						return (
							<Fragment key={index}>
								<p className="mb-5" style={{lineHeight: 1.5}}>{item.text}</p>
						...
						return (
							<Fragment key={index}>
								...
								{item.certificates.map((certificate, certificateIndex) => {
									return (
										<div key={`${index}${certificateIndex}`} className="mb-5">
				...
				<div>
					...
					className="btn btn-primary">{showMore?'Collapse':'Read More...'}</span>
				</div>
			</div>
		</main>
	)
